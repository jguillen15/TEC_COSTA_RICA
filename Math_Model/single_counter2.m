...
t=[3 6]; % time interval
[T, Y] = ode15s(@func_single_counter2,t,y0,options,Dtot,Kir,Kr1,Kr2,Ksyn,Ksynr,KbI,Klri,0.8,0.03,15,0.03);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%b=Y(:,5)-Y(:,4)+Kir;
...
for i=1:length(T)
      Th(i)=T(i);  
      pulse(i)=0.5*(tanh((Th(i)-ara_on-3)/kt)-tanh((Th(i)-ara_off-3)/kt));
end
...
title('Forward recombination Second Integrase')
xlabel('Time(h)') 
